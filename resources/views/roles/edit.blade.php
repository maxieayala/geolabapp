                ...
                <form method="POST" action="{{ route('roles.update', ['role' => $role->id]) }}">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            ...
                            <select class="form-control form-control-user @error('guard_name') is-invalid @enderror"
                                name="guard_name">
                                <option selected disabled>Select Guard Name</option>
                                <option value="web"
                                    {{ old('guard_name') ? (old('guard_name') == 'web' ? 'selected' : '') : ($role->guard_name == 'web' ? 'selected' : '') }}>
                                    Web</option>
                                <option value="api"
                                    {{ old('guard_name') ? (old('guard_name') == 'api' ? 'selected' : '') : ($role->guard_name == 'api' ? 'selected' : '') }}>
                                    Api</option>
                            </select>
                            @error('guard_name')
                                <span class="text-danger">{{ $message }}</span>
                                    ...
                                    @foreach ($permissions as $permissionIndex => $permission)
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input permission-input"
                                                ...
                                                id="inlineCheckbox_{{ $permissionIndex }}" value="{{ $permission->id }}">
                                            <label class="form-check-label"
                                                for="inlineCheckbox{{ $permissionIndex }}">{{ $permission->name }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
